...
export default function DataAnalyticsV2({ suppliers = [], payments = [], testMode = false, mockData = null }) {
  ...
  useEffect(() => {
    ...
    dataSuppliers.forEach(supplier => {
      ...
      if (!categoryStats[category]) {
        categoryStats[category] = { amount: 0, count: 0, suppliers: 0 };
      }
    ...
    const concentrationRisk = (top5Amount / totalSpent) * 100;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="px-4 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-white"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {((analytics.paymentsByAmount.small / analytics.paymentCount) * 100).toFixed(1)}% of total
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-500">$1K - $10K</span>
          </div>
          <div className="text-2xl font-bold text-gray-900 dark:text-white">
            {analytics.paymentsByAmount.medium}
          </div>
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {((analytics.paymentsByAmount.medium / analytics.paymentCount) * 100).toFixed(1)}% of total
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {((analytics.paymentsByAmount.large / analytics.paymentCount) * 100).toFixed(1)}% of total
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white dark:bg-gray-900 rounded-lg p-4">
            <h4 className="font-medium text-gray-900 dark:text-white mb-2">Supplier Concentration Risk</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {analytics.concentrationRisk > 50 
                ? `⚠️ High risk: Top 5 suppliers account for ${analytics.concentrationRisk.toFixed(1)}% of spending. Consider diversifying.`
                : `✅ Healthy diversification: Top 5 suppliers account for ${analytics.concentrationRisk.toFixed(1)}% of spending.`
              }
            </p>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-lg p-4">
            <h4 className="font-medium text-gray-900 dark:text-white mb-2">Payment Optimization</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              💡 {analytics.paymentsByAmount.small} small payments detected. Consider batch processing to reduce transaction costs.
            </p>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-lg p-4">
            <h4 className="font-medium text-gray-900 dark:text-white mb-2">Category Analysis</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              📊 {categoryArray[0].name} is your largest expense category at ${categoryArray[0].amount.toLocaleString()}.
            </p>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-lg p-4">
            <h4 className="font-medium text-gray-900 dark:text-white mb-2">Growth Trend</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              📈 Spending increased by 12.5% compared to the previous period. Monitor budget adherence.
            </p>
          </div>
        </div>
      </div>
