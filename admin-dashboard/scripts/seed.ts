...
import { accounts, transactions, analyticsSnapshots } from "../drizzle/schema";
...
async function seed() {
  ...
  try {
    ...
    const accountStatuses = ["active", "frozen", "suspended", "closed"] as const;
    const kycStatuses = ["verified", "pending", "rejected", "none"] as const;
    const riskLevels = ["low", "medium", "high"] as const;
    const accountTypes = ["personal", "business", "escrow"] as const;
    ...
    const txStatuses = ["pending", "confirmed", "failed", "flagged"] as const;
    const txTypes = ["deposit", "withdrawal", "transfer", "payment"] as const;

    const transactionsData = [];
    for (let i = 1; i <= 200; i++) {
      const status = txStatuses[Math.floor(Math.random() * txStatuses.length)];
      const riskScore = Math.floor(Math.random() * 100);
      const fromAccount = accountsData[Math.floor(Math.random() * accountsData.length)];
      const toAccount = accountsData[Math.floor(Math.random() * accountsData.length)];
    ...
    await db.insert(analyticsSnapshots).values(snapshotsData);
    console.log(`✅ Created ${snapshotsData.length} analytics snapshots`);
