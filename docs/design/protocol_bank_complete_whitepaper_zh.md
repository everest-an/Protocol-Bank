...
PBX 代币作为纯粹的效用代币，具有以下核心用途：

- **安全质押 (Security Staking)：** 用户可以将 PBX 代币质押于 Protocol Bank 安全模块（Safety Module），为网络提供安全保障。在极端情况下（如智能合约漏洞或系统故障），质押的 PBX 可能被部分减记以弥补用户损失。作为回报，质押者将获得协议收入的分成。
- **费用燃烧 (Fee Burn)：** 平台产生的部分交易费用将用于回购并销毁 PBX 代币，形成通缩效应，提升剩余代币的价值。这是一个自动化的、不可更改的机制。
- **流动性激励 (Liquidity Incentive)：** 向 Protocol Bank 的自动做市商（AMMs）提供流动性的用户将获得 PBX 代币奖励，以确保平台的流动性和交易效率。
